#!/usr/bin/env python3
"""
Forward test: $100K capital, $1K per stock, signal date 2026-04-01.
Model uses only data available before market open on 4/1 (i.e., through 3/31).
Entry at 4/1 close, exit at 5/29/2026 close.
"""
import sys, warnings
import pandas as pd
...
# -- Config ---------------------------------------------------------------------
SIGNAL_CUTOFF = "2026-04-01"   # exclusive: model sees data through 3/31
ENTRY_DATE    = "2026-04-01"   # buy at this day's close
...
for idx, ticker in enumerate(UNIVERSE, 1):
    try:
        # Model data: everything through 3/31/2026
        hist = yf.download(ticker, start="2024-01-01", end=SIGNAL_CUTOFF,
        ...
        # Performance window: 4/1 close → 5/29 close
        ...
        entry_price = float(perf["Close"].iloc[0])   # 4/1 close
...
spy_return = (float(spy_data["Close"].iloc[-1]) - float(spy_data["Close"].iloc[0])) / float(spy_data["Close"].iloc[0]) * 100
...
print(f"\n{'='*68}")
print(f"  FORWARD TEST: {ENTRY_DATE} to {EXIT_DATE}")
print(f"{'='*68}")
print(f"  Tickers screened     : {len(results)}")
print(f"  BUY labels (>0.65)   : {sum(1 for r in results if r['label']=='BUY')}")
print(f"  WATCH labels (note)  : {sum(1 for r in results if r['label']=='WATCH')}")
print(f"  AVOID labels (<0.40) : {sum(1 for r in results if r['label']=='AVOID')}")
print()
print(f"  Full 5-layer scores active: Congress(Quiver paid) + NewsAPI + Earnings(yfinance)")
...
print(f"  S&P 500 return    : {spy_return:>+10.2f}%")
...
# Quintile breakdown: does higher score -> higher return?
...
# Rank correlation (Spearman-style: does higher score rank -> higher return rank?)
...
# Trade cards for BUY signals in portfolio
